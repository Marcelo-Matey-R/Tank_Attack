#include <stdexcept>
#include <vector>
#include <random>
#include "Element.h"
#include "StructPosition.h"
#include "EnumColorTank.h"
#include "EnumPowerUp.h"
#include "EnumTypePath.h"
#include "../../UI/include/Constants.h"

#ifndef TANK_H
#define TANK_H
class Bullet;
class Tank : public Element
{
protected:
    std::random_device rng{};
    std::mt19937 gen{rng()};
    std::uniform_int_distribution<int> dis{1, 10};
    float life;
    float angle;
    ColorTank color; // Valor que se utiliza el mapa
    Position position;
    virtual TypePath SelectPathfinding(PowerUp movePrecise = PowerUp::NULL_POWERUP) = 0; // Da un numero para saber cual pathfinding se eligio (depende del tipo de tanque)
public:
    Tank(ColorTank color) : Element(TypeElement::Tank)
    {
        this->color = color;
        this->life = 100;
        Position pos = {0, 0};
        SetPosition(pos);
    }
    virtual ~Tank() = default;
    void SetAngle(float angle) { this->angle = angle; }
    float GetAngle() const { return this->angle; }
    void SetPosition(const Position &n)
    {
        if (n.i < 0 || n.j < 0 || n.i >= GRID_COLS || n.j >= GRID_ROWS)
            throw std::invalid_argument("La posicion no puede ser negativa o sobrepasar los limites del mapa");
        position.i = n.i;
        position.j = n.j;
    }

    virtual void DecreaseHealth(PowerUp attackPlus = PowerUp::NULL_POWERUP) = 0;                                                                     // Cada tipo de tanque tiene en recibir danio
    virtual Bullet *Attack(const Position &destiny,PowerUp attackPlus = PowerUp::NULL_POWERUP, PowerUp attackAccuracy = PowerUp::NULL_POWERUP) = 0; // Cada tanque ataca a su manera
    bool IsAlive() const { return life > 0; }                                                                                                        // Dice si el tanque esta vivo o no
    float GetLife() const { return life; }
    ColorTank GetColor() const { return color; }                                                      // Devuelve el color del tanque
    Position GetPosition() const { return position; }                                                 // Devuelve la posicion
    std::vector<Position> GetPath(Position destination, Element* (&map)[GRID_ROWS][GRID_COLS], PowerUp movePrecise = PowerUp::NULL_POWERUP); // Devuelve el Path para ir al destino
};

// Subclases de tanque
class TankLightBlueAndBlue : public Tank
{
private:
    TypePath SelectPathfinding(PowerUp movePrecise = PowerUp::NULL_POWERUP) override;

public:
    TankLightBlueAndBlue(ColorTank color) : Tank(color) {}
    void DecreaseHealth(PowerUp attackPlus = PowerUp::NULL_POWERUP) override;
    Bullet *Attack(const Position &destiny, PowerUp attackPlus = PowerUp::NULL_POWERUP, PowerUp attackAccuracy = PowerUp::NULL_POWERUP) override;
};

class TankYellowAndRed : public Tank
{
private:
    TypePath SelectPathfinding(PowerUp movePrecise = PowerUp::NULL_POWERUP) override;

public:
    TankYellowAndRed(ColorTank color) : Tank(color) {}
    void DecreaseHealth(PowerUp attackPlus = PowerUp::NULL_POWERUP) override;
    Bullet *Attack(const Position &destiny,PowerUp attackPlus = PowerUp::NULL_POWERUP, PowerUp attackAccuracy = PowerUp::NULL_POWERUP) override;
};

#endif